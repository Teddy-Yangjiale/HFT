...
auto FixedSpreadMarketMaker::on_market_event(const MarketEvent& event, const OrderBook& book) -> std::vector<OrderRequest> {
    ...
    return {
        OrderRequest{symbol_, Side::Buy, OrderType::Limit, bid, order_qty_},
        OrderRequest{symbol_, Side::Sell, OrderType::Limit, ask, order_qty_},
    };
}
